...
import { formatDateShort, formatRelativeTime, getDateRange } from '@/utils/date-utils';
...
const AuthorizationsPage: React.FC<AuthorizationsPageProps> = ({
  ...
  const filteredAuthorizations = useMemo(() => {
    let filtered = authorizations.filter(auth => {
      // Workspace filter
      if (state.filters.workspace !== 'all' && auth.workspace !== state.filters.workspace) {
        return false;
      }

      // Data source filter
      if (state.filters.dataSource !== 'all' && auth.type !== state.filters.dataSource) {
        return false;
      }

      // Status filter
      if (state.filters.status !== 'all' && auth.status !== state.filters.status) {
        return false;
      }
  ...
  const handleSelectAll = (checked: boolean) => {
    setState(prev => ({
      ...prev,
      selectedItems: checked ? filteredAuthorizations.map(auth => auth.id) : []
    }));
  };
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between items-center pt-2 border-t">
            <div className="text-sm text-gray-600">
              {filteredAuthorizations.length} authorization{filteredAuthorizations.length !== 1 ? 's' : ''} found
              {state.selectedItems.length > 0 && (
                <span className="ml-2">• {state.selectedItems.length} selected</span>
              )}
            </div>
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-12">
                  <Checkbox
                    checked={
                      state.selectedItems.length === filteredAuthorizations.length &&
                ...
                <TableRow key={auth.id} className="hover:bg-gray-50">
                  <TableCell>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => handleSelectItem(auth.id, !!checked)}
                    />
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <span className="text-lg">{getDataSourceIcon(auth.type)}</span>
                  ...
                  <TableCell className="text-sm text-gray-600">
                    {auth.lastUsed ? formatRelativeTime(auth.lastUsed) : 'Never'}
                  </TableCell>
                  <TableCell>
                    <span className="font-medium">{auth.permissionsCount}</span>
                  </TableCell>
                  <TableCell>
                    <span className="font-medium">{auth.datastreamsCount}</span>
                  </TableCell>
                  <TableCell>
