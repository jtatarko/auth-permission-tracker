...
function EmailPage() {
  ...
  const handleSeeDetails = (authId: string, dateRange: { from: Date; to: Date }) => {
...
function AuthorizationDetailWrapper() {
  ...
  const authorization = id ? authorizations.find(auth => auth.id === id) : null;
    ...
    <Router>
      <Layout>
        <Routes>
          ...
          <Route path="/authorizations" element={<AuthorizationsPageWrapper />} />

          {/* Authorization Detail Route */}
          <Route path="/authorization/:id" element={<AuthorizationDetailWrapper />} />
